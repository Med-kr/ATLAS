        ...
        <div class="p-6 rounded-2xl backdrop-blur-xl bg-white/5 border border-white/10">
            <form action="{{ route('tags.update', $tag) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $tag->name) }}" class="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/10 focus:border-purple-400 outline-none transition text-white placeholder-gray-500" placeholder="Ex: PHP, JavaScript, Laravel" required>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ route('tags.index') }}" class="flex-1 py-3 rounded-xl bg-white/10 border border-white/10 text-center hover:bg-white/20 transition">Annuler</a>
                </div>
            </form>
        </div>
    </div>
